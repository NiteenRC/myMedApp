angular.module('myCart.productManage_module', ['myCart.shared_module.sharedService', ]).controller('productManageController', productManageController);
...
function productManageController($scope, $uibModal, sharedService) {
  ...
  function placeOrderSelectedItem() {
    ...
    sharedService.postMethod(productUrl + 's', productList).then(
      function(response) {
        getProducts();
      },
      function(error) {
  ...
  function removeProduct(id) {
    sharedService.deleteMethod(productUrl + '/' + id).then(
      ...
      function(error) {
        $scope.errorMessage =
          'Error while creating User: ' + error.data.errorMessage;
        $scope.successMessage = '';
      }
    );
  }
  ...
  function getProducts() {
    sharedService.getAllMethod(productUrl).then(
      function(response) {
